...
import requestWebPerformanceResult from '../services/api/api';
...
export default function WebPerformance() {
  ...
  const handleClickDetermineWebPerformance = async () => {
    console.log(`hi`);
    const webPerformanceResult = await requestWebPerformanceResult('https://www.naver.com');
    console.log(`hi im ${JSON.stringify(webPerformanceResult)}`);
  };
  ...
  return (
    <div>
      ...
      <div>
        <button onClick={handleClickDetermineWebPerformance}>측정하기</button>
      </div>
    </div>
  );
}
